...
Inlining embeds the compiled partial body in the parent artifact but retains the partial's isolated `RenderContext` boundary; it must preserve render and stream chunk behavior, output bags, template and line exception metadata, resource limits, interrupts, and current error handling.

The parent artifact identity includes transitive dependency content hashes and compiler and artifact format versions. The application owns invalidation and must rebuild affected parents, publish a consistent artifact set atomically or through a versioned artifact directory, and never activate a parent with stale inlined dependencies.
